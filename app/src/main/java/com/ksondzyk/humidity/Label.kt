...
class Label(
    ...
    private fun onYChange() {
        if (!inFocus) {
            progressData.apply {
                ...
                progress = Math.round(progress).toFloat()
            }
        } else {
            progress = Math.round(view.getProgress()).toFloat()
        }

        lastCharIndex = (Math.log10(progress.toDouble()) + 1).toInt()
    ...
    fun inFocus(view: View) {
        ...
        animator?.apply {
            ...
            addUpdateListener { animation ->
                ...
                paint.color = intermediateColor(color,
    ...
    fun outFocus(view: View) {
        ...
        animator?.apply {
            ...
            interpolator = this@Label.interpolator
            addUpdateListener { animation ->
                paint.color = intermediateColor(selectedColor,
